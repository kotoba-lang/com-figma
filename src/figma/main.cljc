(ns figma.main
  "Kotodama WASM entrypoint for the Figma clean-room actor (L4) — Clojure port.
  ...
  facts (`figma.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "File"      :plural "files"      :id-prefix "figma_fil"
    :fields [:name :role :lastModified :editorType :version :schemaVersion] :required [:name :role]
    :coerce {:schemaVersion :int}     :refs {}}
   {:entity "Node"      :plural "nodes"      :id-prefix "figma_nod"
    :fields [:name :type :visible :rotation :locked]        :required [:name]
    :coerce {:visible :bool :rotation :float :locked :bool} :refs {}}
   {:entity "Component" :plural "components" :id-prefix "figma_com"
    :fields [:key :name :description]        :required [:key :name]
    :coerce {}                       :refs {}}
   {:entity "Comment"   :plural "comments"   :id-prefix "figma_com"
    :fields [:fileKey :message :resolvedAt]  :required [:fileKey :message]
    :coerce {}                       :refs {}}
   {:entity "Project"   :plural "projects"   :id-prefix "figma_pro"
    :fields [:name :teamId]                  :required [:name]
    :coerce {}                       :refs {}}
   {:entity "Style"     :plural "styles"     :id-prefix "figma_sty"
    :fields [:key :name :styleType]          :required [:key :name]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"figma.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "figma-compat" :tier tier :entities entities} 200])
